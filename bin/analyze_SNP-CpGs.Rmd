...
params:
  mae_hdf5_path: NULL
  SNP_CpG_Dir: #Z:/omics/Methylation/Daniel_files/Outputs_SNP_CpG_DB
...
df_list <- lapply(unique(SNPs_overlapping_prot$Gene_symbol), function(x) {
  ...
  genotype_path <- file.path(SNP_CpG_Dir, paste0("Chr", Chr), paste0("snps_genotypes", Chr, '.csv'))
...
plot_sign_results <- function(lm_results, row_n, outdir2) {
  ...
  ggsave(file.path(outdir2, paste0(gene, "_", ipp.id, ".pdf")), p, width = 6, height = 4)
  return(p)
...
for (i in nrow(rna_snp_counts_sign_with_prot)) {
  ...
  ggsave(file.path('SNP_CPG_RNA', paste(gene, "_", i, ".pdf")), p, width = 6, height = 4)
  print(p)
