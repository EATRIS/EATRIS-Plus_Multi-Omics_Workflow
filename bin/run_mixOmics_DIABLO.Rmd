...
```{r}
# merge targeted metabolomics assays
# tmp_mtblms_se <- list(mae[["Amino acids"]], mae[["Acylcarnitines"]],
#                   mae[["Very long chain fatty acids"]])
# tmp_mtblms_se <- lapply(tmp_mtblms_se, function(se) {
#   rowdata_df <- data.frame(rowData(se))
#   if (!"HMDB.ID.s." %in% names(rowdata_df)) {rowdata_df$HMDB.ID.s. <- NA}
#   se <- SummarizedExperiment(assays = assays(se), rowData = rowdata_df)
# })
# 
# mae <- c(mae, `Metabolomics` = do.call(rbind, tmp_mtblms_se))
```
...
pca.X <- lapply(assay_names, function(assay_name) {
  print(assay_name)
  df <- as.data.frame(X[[assay_name]])
  ## Remove non-variabe columns
  df <- df[, apply(df, 2, function(x) length(unique(x)) > 1)]
  pca(df, ncomp = n_comp,  center = TRUE, scale = TRUE)
})
...
lapply(assay_names, function(assay_name) {
  plotIndiv(pca.X[[assay_name]], group = Y, ind.names = FALSE, legend = TRUE, ellipse = T,
...
Run DIABLO using all features 

```{r}
initial.diablo.XY <- block.splsda(X, Y, ncomp = n_comp, design = design)
...
Optimize number of components based on cross-validation

```{r}
# run performance evaluation
perf.initial.diablo.XY <- perf(
  initial.diablo.XY, 
  # repeated 5-fold cross-validation
  validation = "Mfold", folds = 5, nrepeat = params$n_repeat, 
  progressBar = FALSE)

# plot performance evaluation 
plot(perf.initial.diablo.XY)

print(perf.initial.diablo.XY$choice.ncomp)

choice.ncomp <- perf.initial.diablo.XY$choice.ncomp$WeightedVote[
  "Overall.BER", "centroids.dist"]
```

Tune number of features (for sparse model)

Run final model

```{r}
final.diablo.XY <- block.splsda(X, Y, ncomp = choice.ncomp)
```
...
```{r}
if (choice.ncomp > 1) {
  plotIndiv(final.diablo.XY, ind.names = F, legend = T)
}
```
...
```{r}
plotDiablo(final.diablo.XY)
```


```{r}
# plotArrow(final.diablo.XY, ind.names = F, legend = T)
# no spaces allowed in assay name?
```


```{r}
if (choice.ncomp > 1) {
  plotVar(final.diablo.XY, var.names = F, legend = T, cutoff = 0.5)
}
        # col = ggokabeito::palette_okabe_ito(1:length(X)))
```


```{r, fig.width=14}
num_features2plot <- 10
lapply(1:choice.ncomp, function(comp) {
  plotLoadings(final.diablo.XY, 
               ndisplay = num_features2plot, 
               size.title = 1,
               comp = comp,
...
```{r, fig.width=10}
# network(final.diablo.XY, cutoff = 0.4)
```
...
Determine performance of final model

```{r}
perf.final.diablo.XY <- perf(
  final.diablo.XY, 
  validation = "Mfold", folds = 5, nrepeat = params$n_repeat, 
...
```{r}
perf.final.diablo.XY$MajorityVote.error.rate
```

```{r}
perf.final.diablo.XY$WeightedPredict.error.rate
```
...
```{r}
lapply(assay_names, function(assay_name) {
  auc.diablo.XY <- auroc(final.diablo.XY, roc.block = assay_name)
  NULL
})
```

```{r}
final.diablo.XY.loadings <- final.diablo.XY$loadings

lapply(assay_names, function(a) {
  df <- cbind(final.diablo.XY.loadings[[a]], data.frame(rowData(mae[[a]])))
  f <- gsub("[| ,]", "_", a)
  write.csv(df, file = file.path(outdir, paste0("XYloadings_", f, ".csv")))
})
```
