...
require(vioplot)
require(sm)
violinplot<-function (x, ...,xlab="",ylab="", main="", range = 1.5, h = NULL, ylim = NULL, names = NULL, 
    horizontal = FALSE, col = "magenta", border = "black", lty = 1, 
    lwd = 1, rectCol = "black", colMed = "white", pchMed = 19, 
    at, add = FALSE, wex = 1, drawRect = TRUE, cex.axis = 1,las=2) 
{
    if(!is.list(x)){
        datas <- list(x, ...)
    } else{
    ...
    if (!horizontal) {
        if (!add) {
            ...
            axis(1, at = at, label = label, las = las,cex.axis=cex.axis  )
        }
        box()
        for (i in 1:n) {
